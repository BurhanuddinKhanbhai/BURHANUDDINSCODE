...
@Disabled
@Autonomous(name = "CycloneAutoBlue_RR", group = "Robot")
public class BlueAutoRR extends LinearOpMode {
    ...
    private static final double INTAKE_FWD_IN = 23;   // forward while intaking
    ...
    public void runOpMode() {
        ...
        Action auto = drive.actionBuilder(startPose)
                ...
                .strafeTo(new Vector2d(0, 30))       // strafe (Y coord)
        ...
        pose = doLineMoveX(drive, pose, +INTAKE_FWD_IN+5);
        stopIntake();

        // 5) DRIVE BACK (undo intake move)
        pose = doLineMoveX(drive, pose, -INTAKE_FWD_IN-10);
        ...
        pose = doTurn(drive, pose, Math.toRadians(273));

        pose = doLineMoveX(drive, pose, -15);

        pose = doTurn(drive, pose, Math.toRadians(-200));
        ...
        pose = doTurn(drive, pose, Math.toRadians(-60));
    ...
    private void runActionBlocking(MecanumDrive drive, Action auto) {
    }
